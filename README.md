...
- 한국문학 문장이 1,440분을 모두 채우고 있지만, **문장 속 시각이 현재 분과 정확히
  일치하는 것은 117분**입니다. 나머지 분에는 같은 시간대(새벽·아침·저녁 등)의 문장이
  표시되며, 화면에서 구분 없이 자연스럽게 보입니다.
- 분 단위 정확 매칭 데이터의 다수는 현재 **영미문학**([데이터 레포 `data/en/`](https://github.com/parkds-claude/k-text-clock-web/tree/main/data/en),
  1,440분 중 95% 커버)입니다.
- **시각이 등장하는 한국문학 문장을 계속 수집하고 있습니다** — 읽던 책에서
  발견하셨다면 [기여](#문장-기여)해 주세요.
